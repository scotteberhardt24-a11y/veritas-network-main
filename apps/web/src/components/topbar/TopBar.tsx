          ...
          {showMenu && (
            <div style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", background: "rgba(4,15,36,0.99)", border: "1px solid rgba(26,107,255,0.2)", borderRadius: 10, padding: 6, minWidth: 160, zIndex: 50, boxShadow: "0 8px 32px rgba(0,0,0,0.4)" }}>
              {[
                ...
                <button key={i} onClick={()=>{item.action();setShowMenu(false);}} style={{ width:"100%", padding:"8px 12px", background:"transparent", border:"none", borderRadius:7, color:item.danger?"#ff7777":"rgba(255,255,255,0.7)", fontSize:"0.8rem", fontWeight:500, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:8 }}
